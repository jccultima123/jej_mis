<div class="container">
    <h3><span class="glyphicon glyphicon-wrench"></span>&nbsp;Setup Wizard</h3><hr />

    <section>
        <div class="row">
            <div class="col-md-6 text-center">
                <a href="install/">
                    <h1><i class="glyphicon glyphicon-log-in"></i></h1>
                    <h3>Install / Reconfigure</h3>
                </a>
                <p class="text-muted">If you are about to start or got troubled in installation,<br />you may go through here</p>
            </div>
            <div class="col-md-6 text-center">
                <a href="recovery/">
                    <h1><i class="glyphicon glyphicon-alert"></i></h1>
                    <h3>Recovery and Report</h3>
                </a>
                <p class="text-muted">System got crashed? You may flush all the data or send details here</p>
            </div>
        </div>
    </section>

</div>